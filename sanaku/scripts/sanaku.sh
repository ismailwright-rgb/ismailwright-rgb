...
' "$_bodies"
  ...
  if [ "$_rc" -eq 0 ]; then
    ...
    say ""
    say "  A first-name-only match here (e.g. \"Steve\" not \"Steve Adler\") is Apollo,"
    say "  not a bug: search results obfuscate last names unless the plan includes"
    say "  full-reveal-in-search - confirmed live, and documented at"
    say "  docs.apollo.io/reference/people-api-search. This preflight only searches,"
    say "  never reveals, so it will show this even on a plan with full reveal."
    say "  A real scrape does not stay stuck there - W1 upgrades a partial name using"
    say "  the SAME paid reveal call that already gets the email, whenever that reveal"
    say "  has a fuller name than search did."
    say ""
