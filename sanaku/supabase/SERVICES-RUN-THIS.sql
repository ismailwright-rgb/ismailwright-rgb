...
--   018  Track an async Apollo phone reveal (8 credits, minutes to resolve)
--        across runs. request_id stored as TEXT - it can exceed 2^53.
--   019  A personal email column, kept separate from the one W2 sends from.
--
...
-- ####################  migration-018-phone-reveal.sql  ####################
...
-- ####################  migration-019-personal-email.sql  ####################
...
select
  ...
  (select count(*) from information_schema.columns
    where table_name = 'sanaku_prospects'
      and column_name in ('phone_reveal_request_id',
                          'phone_reveal_requested_at'))              as phone_reveal_cols,-- expect 2
  (select count(*) from information_schema.columns
    where table_name = 'sanaku_prospects'
      and column_name = 'contact_email_personal')                    as personal_email_col; -- expect 1
